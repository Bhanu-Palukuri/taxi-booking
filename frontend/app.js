...
// ✅ Fetch all pending rides for drivers
function fetchPendingRides() {
    fetch(`${backendURL}/rides/`)
        .then(response => response.json())
        .then(data => {
            const tableBody = document.getElementById("availableRides");
            tableBody.innerHTML = ""; // Clear previous data

            if (data.length === 0) {
                tableBody.innerHTML = "<tr><td colspan='3'>No available rides</td></tr>";
                return;
            }

            data.forEach(ride => {
                if (ride.ride_status === "Pending") {  // Only show pending rides
                    let row = `<tr>
                        <td>${ride.pickup_location}</td>
                        <td>${ride.drop_location}</td>
                        <td>
                            <button onclick="acceptRide(${ride.id})">✅ Accept</button>
                            <button onclick="cancelRide(${ride.id})">❌ Cancel</button>
                        </td>
                    </tr>`;
                    tableBody.innerHTML += row;
                }
...
function acceptRide(rideId) {
    fetch(`${backendURL}/rides/${rideId}/`, {
        method: "PATCH",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ ride_status: "Accepted" })
    })
    .then(response => response.json())
    .then(() => {
...
function cancelRide(rideId) {
    ...
    .catch(error => console.error("Error cancelling ride:", error));
}

// ✅ Call fetchPendingRides when driver panel is opened
if (document.getElementById("availableRides")) {
    fetchPendingRides();
}